# chat-compact Specification

## Purpose
TBD - created by archiving change add-chat-compact. Update Purpose after archive.
## Requirements
...
系统 SHALL 将 compact 结果写回为一个 compacted turn，而不是写入 memory。该 compacted turn SHALL 包含两条普通 chat message：
- compacted `assistant` message：明确说明“这是压缩后的上下文”，并保留任务目标、用户约束、当前背景、当前规划、已完成、未完成和关键事实
- follow-up `user` message：固定写入“继续”，让后续对话自然续接

#### Scenario: compact 结果保留为 assistant summary + user continue
- **WHEN** 当前线程完成一次 compact
- **THEN** 系统会在 chat 历史中写入一个包含 compacted `assistant` 和 follow-up `user` 的 compacted turn
- **THEN** 这两条 message 可以被后续对话继续使用

### Requirement: compacted turn SHALL 参与后续 compact
系统 SHALL 将 compact 生成的 compacted turn 视为后续 chat history 的一部分。在未来再次 compact 时，已有 compacted turn SHALL 和后续新增 chat 一起参与压缩。

#### Scenario: 再次 compact 会包含旧 compacted turn
- **WHEN** 某个线程已经存在 compacted turn 且后续又产生新的 chat history
- **THEN** 下一次 compact 时，该 compacted turn 会作为输入的一部分参与新的 compact

### Requirement: 首版 SHALL 直接替换被 compact 的 active chat history
首版 compact 完成后，系统 SHALL 从 active chat history 中移除被 compact 的旧 chat messages，并用新的 compacted turn 替换它们。系统设计 SHALL 为未来 archive / shadow copy 策略保留扩展点，但首版 active history 中 SHALL NOT 保留被替换的旧 chat messages。

#### Scenario: 旧 chat 被 compacted turn 替换
- **WHEN** 首版 compact 成功执行
- **THEN** 被 compact 的旧 chat messages 不再出现在 active chat history 中
- **THEN** active chat history 只保留 compacted turn 及其后的新消息

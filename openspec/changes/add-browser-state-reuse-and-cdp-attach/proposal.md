...
- 为工作区内的浏览器持久化状态文件增加统一落盘约定，推荐收敛到 `.openjarvis/browser/`，首版实际落盘的是 cookies-only 文件，并默认加入 gitignore，避免把登录态状态文件误提交到仓库。
...
- Runtime: sidecar 将同时支持“启动新浏览器”和“attach 到现有浏览器”两条执行路径，并在 close 生命周期中按配置处理 cookies 自动导出；工作区约定的浏览器持久化状态文件推荐统一放在 `.openjarvis/browser/`，其中首版实际导出的是 cookies-only 文件。

...
`context/` 负责系统内部统一消息协议建模，以及和上下文预算相关的基础概念。
...
- `ChatMessageRole`
  单条消息在上下文中的角色类型，例如 `system`、`memory`、`user`、`assistant`、`toolcall`、`tool_result`。
- `ChatMessage`
  统一后的单条上下文消息，是后续传给 LLM、写入 Thread 的基础单元。
- `ChatToolCall`
...
- Router、Worker、AgentLoop 主链路都不在这里组装请求上下文。

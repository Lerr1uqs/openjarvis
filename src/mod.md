...
`src/` 是 OpenJarvis 的主运行时入口层，负责把外部消息接进来，解析线程上下文，交给 Agent 执行，再把结果发回外部渠道。
...
- `compact`
  上下文压缩层。负责在上下文过大时，把旧消息历史折叠成更小的替代消息。
- `config.rs`
  配置加载层。负责应用配置、Agent 配置、Tool 配置、MCP 配置、Hook 配置、LLM 配置。
- `context`
  统一消息协议层。负责 `ChatMessage` 等基础类型和预算桶概念。
- `llm.rs`
...
- `thread.rs`
  线程历史层。负责描述 `Thread`、`ThreadContext`、`ThreadToolEvent` 这些线程对象。
- `main.rs`
...
- `Turn`
  一次用户输入触发的一轮处理结果，当前只保留为提交概念。
- `AgentWorker`
  长生命周期执行体。负责接收路由层请求，并把单轮任务交给 `AgentLoop`。
- `AgentLoop`
  单轮 ReAct 执行器。负责调 LLM、调用工具、汇总最终输出。
- `ToolRegistry`
  工具注册中心。负责维护全局工具目录、工具集、MCP 工具与技能加载；线程级可见性和加载状态由 `Thread` 负责。
- `Compact`
